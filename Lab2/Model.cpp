...
void Model::Draw(ShaderProgram* program,glm::vec3 cameraPosition)
{
	...
	for (GLuint i = 0; i < this->meshes.size(); i++)
	{
		...
		//todo:ƒобавить сортировку прозрачных объектов по рассто€нию от камеры
...
Mesh Model::processMesh(aiMesh * mesh, const aiScene * scene)
{
	...
	Material meshMaterial;
		...
		if (mesh->mMaterialIndex >= 0)
		{
			...
			//	alpha = 1;//если не удалось получить значение, то полностью непрозрачный
			//}
			if (aiGetMaterialFloat(material, AI_MATKEY_OPACITY, &meshMaterial.alpha) != AI_SUCCESS) {
				meshMaterial.alpha = 1;//если не удалось получить значение, то полностью непрозрачный
			}
			...
			{
				//«агрузить текстуру по умолчанию
...
//todo:—делать загрузку по относительным пут€м
